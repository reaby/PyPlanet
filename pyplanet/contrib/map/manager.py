...
class MapManager(CoreContrib):
    ...
    async def handle_map_change(self, info):
        ...
        map_info = await Map.get_or_create_from_info(
            ...
            file=info['FileName'], environment=info['Environnement'], map_type=info['MapType'],
            map_style=info['MapStyle'],
    ...
    async def update_list(self, full_update=False, detach_fks=True):
        ...
        if full_update:
            ...
            for existing_map in [m for m in maps if m.author_nickname is None or len(m.author_nickname) == 0 or (
                m.mx_id is None and "MX" in m.file)]:
    ...
    async def upload_map(self, fh, filename, insert=True, overwrite=False):
        ...
        data = fh.read(-1)
        await self._instance.gbx("WriteFile", f"{filename}", data)
        fh.close()
    ...
    async def _override_timelimit(self, filename):
        ...
        if self._is_extended and self._original_ta:
            try:
                async with self._instance.storage.open(os.path.join(self._instance.game.server_map_dir, filename),
                                                       'r+') as f:
                ...
                logging.getLogger(__name__).warning(
                    'Can\'t update matchsettings with original time limit to \'{}\'!'.format(filename))
    ...
    async def load_matchsettings(self, filename):
        ...
        try:
            ...
            raise MapException(
                'Can\'t load matchsettings according the dedicated server, tried loading from \'{}\'!'.format(
                    filename)) from e
