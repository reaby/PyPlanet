...
class ListView(TemplateView):
    ...
    async def close(self, player, *args, **kwargs):
		...
		"""
        other_manialink = self.manager.instance.ui_manager.get_manialink_by_id(self.id)
...
class ManualListView(ListView):
        ...
        for field in await self.get_fields():
            if 'searching' in field and field['searching']:
                if 'search_strip_styles' in field and field['search_strip_styles']:
                    query.append(
                        [self.search_text.lower() in style.style_strip(str(x).lower()) if x else False for x in
                         (x[field['index']] for x in frame)]
                    )
                else:
                    query.append(
                        [self.search_text.lower() in str(x).lower() if x else False for x in
                         (x[field['index']] for x in frame)]
                    )
        if query:
    ...
    def sort(self, e):
        value = ""
        if isinstance(e[self.sort_field['index']], numbers.Number):
            value = 0
        return e[self.sort_field['index']] or value

    async def apply_ordering(self, frame):
        if self.sort_field:
            frame.sort(key=self.sort,
                       reverse=not bool(self.sort_order))
        return frame
