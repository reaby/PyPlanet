...
class _BaseUIManager:
    ...
    async def send(self, manialink, players=None, **kwargs):
        ...
        attachid = getattr(manialink, 'attachid')
        ...
        if not is_global:
            for login in for_logins:
                ...
                xml = '<manialink version="{}" id="{}" layer="{}" name="{}" attach="{}">{}</manialink>'.format(
        ...
        if self.instance.game.game == 'sm' and manialink.disable_alt_menu:
            if is_global:
                queries.extend([
                    self.instance.gbx('Maniaplanet.UI.SetAltScoresTableVisibility', player.login, 'false',
                                      encode_json=False, response_id=False)
                    for player in self.instance.player_manager.online
                ])
            else:
                queries.extend([
                    self.instance.gbx('Maniaplanet.UI.SetAltScoresTableVisibility', login, 'false', encode_json=False,
                                      response_id=False)
                    for login in for_logins
                ])
    ...
    async def hide(self, manialink, logins=None):
        ...
        if logins and len(logins) > 0:
            ...
            if self.instance.game.game == 'sm' and manialink.disable_alt_menu:
                queries.extend([
                    self.instance.gbx('Maniaplanet.UI.SetAltScoresTableVisibility', login, 'true', encode_json=False,
                                      response_id=False)
                    for login in logins
                ])
        else:
            ...
            if self.instance.game.game == 'sm' and manialink.disable_alt_menu:
                queries.extend([
                    self.instance.gbx('Maniaplanet.UI.SetAltScoresTableVisibility', player.login, 'true',
                                      encode_json=False, response_id=False)
                    for player in self.instance.player_manager.online
                ])
