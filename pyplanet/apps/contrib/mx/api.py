...
class MXApi:
    ...
    async def search(self, titlepack, name=None, author=None, **kwargs):
        ...
        for info in json['Results']:
            info['Difficulty'] = self.difficulties[info['Difficulty']] if info[
                                                                              'Difficulty'] in self.difficulties else 'Unknown'
            info['Environment'] = self.environments[info['Environment']] if info[
                                                                                'Environment'] in self.environments else 'Unknown'
            info['Length'] = times.format_time(int(info['Length'])) if 'Length' in info and info[
                'Length'] is not None else None
            maps.append(info)
        return maps
