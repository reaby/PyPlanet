...
class MapAdmin:
    ...
    async def on_start(self):
        await self.instance.permission_manager.register('previous', 'Skip to the previous map', app=self.app,
                                                        min_level=1)
        ...
        await self.instance.permission_manager.register('add_local_map', 'Add map from server disk', app=self.app,
                                                        min_level=2)
        await self.instance.permission_manager.register('remove_map', 'Remove map from server', app=self.app,
                                                        min_level=2)
        await self.instance.permission_manager.register('write_map_list', 'Write Matchsettings to file', app=self.app,
                                                        min_level=2)
        await self.instance.permission_manager.register('read_map_list', 'Read and load specific Matchsettings file',
                                                        app=self.app, min_level=2)
        ...
        await self.instance.command_manager.register(
            ...
            Command(command='restart', aliases=['res', 'rs'], target=self.restart_map, perms='admin:restart',
                    admin=True,
            ...
            Command(command='local', namespace='add', target=self.add_local_map, perms='admin:add_local_map',
                    admin=True,
            ...
            .add_param('nr', required=False, type=int, help='The number from a list window or the unique identifier.'),
            Command(command='writemaplist', aliases=['wml'], target=self.write_map_list, perms='admin:write_map_list',
                    admin=True,
            ...
            Command(command='readmaplist', aliases=['rml'], target=self.read_map_list, perms='admin:read_map_list',
                    admin=True,
    ...
    async def list_action_remove(self, player, values, map_dictionary, view, **kwargs):
        ...
        cancel = bool(
            await ask_confirmation(player, 'Are you sure you want to remove the map \'{}\'$z$s from the server?'.format(
                map_dictionary['name']
            ), size='sm'))
        if cancel is True:
            return
    ...
    async def extend(self, player, data, **kwargs):
        ...
        try:
            ...
            return await self.instance.chat('$ff0Error: Game mode must be Time Attack to use the extend functionality!',
                                            player)
