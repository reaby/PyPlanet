...
class PyPlanetConfig(AppConfig):
    ...
    async def on_start(self):
        ...
        # Permissions.
        await self.instance.permission_manager.register('upgrade', 'Upgrade the PyPlanet instance', app=self,
                                                        min_level=3)
        await self.instance.permission_manager.register('help', 'Open the commands list for admins', app=self,
                                                        min_level=1)
        ...
        await self.instance.command_manager.register(
            ...
            Command('upgrade', self.chat_upgrade, perms='core.pyplanet:upgrade', admin=True,
            ...
            Command('help', target=self.chat_help, perms='core.pyplanet:help', admin=True,
                    description='Shows a chat-based list with all available admin commands.')
            .add_param('command', nargs='*', required=False),
            Command('helpall', target=self.chat_helpall, description='Shows all available commands in a list window.'),
            Command('helpall', target=self.chat_helpall, perms='core.pyplanet:help', admin=True,
                    description='Shows all available admin commands in a list window.')
        )
    ...
    async def chat_version(self, player, *args, **kwargs):
        message = '$ff0PyPlanet: $fff{}$ff0 (Python $fff{}$ff0), current apps: $fff'.format(version,
    ...
    async def chat_upgrade(self, player, data, *args, **kwargs):
        ...
        await self.instance.gbx.multicall(
            ...
            self.instance.chat(
                '$09fUpgrade$fff: Please wait, installing new version with $09fpip$fff...'.format(to_version))
        )
